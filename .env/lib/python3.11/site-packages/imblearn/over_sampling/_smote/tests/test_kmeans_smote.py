...
@pytest.mark.parametrize(
    ...
    [
        3,
        KMeans(n_clusters=3, n_init=1, random_state=45),
        MiniBatchKMeans(n_clusters=3, n_init=1, random_state=45),
    ],
)
...
def test_sample_kmeans_not_enough_clusters(data):
    X, y = data
    smote = KMeansSMOTE(cluster_balance_threshold=10, random_state=45)
...
def test_sample_kmeans_density_estimation(density_exponent, cluster_balance_threshold):
    X, y = make_classification(
        n_samples=10_000, n_classes=2, weights=[0.3, 0.7], random_state=45
    )
    smote = KMeansSMOTE(
        kmeans_estimator=MiniBatchKMeans(n_init=1, random_state=45),
        random_state=0,
